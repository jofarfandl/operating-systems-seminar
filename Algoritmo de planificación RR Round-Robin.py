...
from re import X#, terminal_size
#import string
#import math
...
#[ID][T_dur][num1][ope][num2][res][T llega][T LLEGADA][T FINALIZADO][T RETORNO][T RESPUESTA][T ESPERA][T SERVICIO][NOSEUTILIZA][Quantum]
...
def llenado(lista, num):
    ...
    for i in range(num):
        ...
        lista.append((id,tiempo,num1,operador,num2,Resultado,0,0,0,0,0,0,0,0,0))
    return num
...
def actuales(lista_aux,lista):#aqui mostramos los procesos pendientes en lista pequeña
    ...
    while (contador < longitud ):
        print(str(lista_aux[contador][0]) + "\t" + str(lista_aux[contador][1]) + "\t" + str(lista_aux[contador][6]))
...
def main():
    ...
    while(len(lista_aux) != 0 or banBloqueo == True):#a partir de aqui ejecuta los procesos
        ...
        while TR != -1:
            ...
            TT = TT + 1
            TR = TR - 1
            proceso[14] = proceso[14] + 1 #suma de 1 en 1 para procesar el quantum
            if(proceso[14] == quantum):#si el numero del quantum es igual al definido, lo regresamos a la cola de procesos
                proceso[14] = 0
                proceso[6] = TT
