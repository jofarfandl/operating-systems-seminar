...
def actuales(lista_aux,lista,suspendidos):	#aqui mostramos los procesos pendientes en lista pequeña
	...
	cont = 0
	for i in suspendidos:
		nueva = list(i)
		nueva[11] = i[11] + 1
		del suspendidos[cont]
		suspendidos.insert(cont,nueva)
		cont += 1
...
def main():
	...
	while(len(lista_aux) != 0 or banBloqueo == True):	#a partir de aqui ejecuta los procesos
		...
		while TR != -1:
			actuales(lista_aux,lista,suspendidos)
			...
			if (tecla == 'i'):
				...
				print("Regresa")
				lista.append(suspendidos[0])
				suspendidos.pop(0)
				time.sleep(1)
				print ("\033[A                                                                                                           \033[A")
				
				file1=open("suspendidos.txt","w+")
				file1.write(str(suspendidos))
				file1.write("\n")
				file1.close()
				if( marcos > 0 and len(lista) > 0 ):
					operacion = ceil(lista[0][15] / 4)
